...
# 3. 创建优化后的LSTM模型并训练
model = Sequential()
model.add(Bidirectional(LSTM(50, return_sequences=True), input_shape=(X.shape[1], X.shape[2])))
...
model.fit(X, Y, epochs=200, batch_size=1, verbose=1, validation_split=0.1, callbacks=[early_stopping])
